...
if __name__ == '__main__':
    ...
    weights = compute_weights(x,weights,iterations=iterations,
                              learning_rate=learning_rate, debug_step=1000)

    # output equation
    print('Equation for logistic regression from 2017: ')
    print('y = ' + str(round(weights[0], 4)) + ' + (' + str(
        round(weights[1], 4)) + ' * mu) + (' + str(
        round(weights[2], 4)) + ' * sigma)')
    print('z(y) = 1 / (1 + e^(-y))')
    ...
    print('Percent Accuracy for 2018 predicted values', round(
        percent_accuracy, 2))

    print('\n__________Question 3__________')

    # calculate true/false positive/negative values for 2018 predictions
    tp_sum = pred_2018[['Color', 'Predicted_Color']].apply(hf.tp, axis=1).sum()
    fp_sum = pred_2018[['Color', 'Predicted_Color']].apply(hf.fp, axis=1).sum()
    tn_sum = pred_2018[['Color', 'Predicted_Color']].apply(hf.tn, axis=1).sum()
    fn_sum = pred_2018[['Color', 'Predicted_Color']].apply(hf.fn, axis=1).sum()
    ...
    # set df_2018 color to predicted color so color_strat_log uses pred values
    ...
    print("\nBuy & Hold strategy results in a larger amount at"
          " the end of the year")
